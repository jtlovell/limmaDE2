...
#' @param what Character vector defining a string to look for in the column names. For example, "q.value"
#' @param na.include Logical, should NAs be classified as not significant? If false, all rows with any NAs are dropped.
#' @param verbose Logical, should the number of significant values be printed
#' @details Given a matrix with p/q values, this function greps for the string in "what" and classifies each column as
#' either significant (1) or not (0). Esspecially useful for a dataset that has many p-value or q-value columns

#' @return a dataframe with all values in the original matrix with column names containing what, where
